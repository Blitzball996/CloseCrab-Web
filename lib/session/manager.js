...
// Returns the set of PIDs for CloseCrab processes that are actually running
// right now. Used to validate stale port-discovery files: a window that was
// closed hard (crash / taskkill / killed console) leaves its port file behind,
// so we must never trust the file alone.
function liveCrabPidSet() {
  const live = new Set();
  if (os.platform() === 'win32') {
    ...
    try {
      const out = execSync('pgrep -f closecrab', { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
      for (const l of out.split('\n')) { const p = parseInt(l.trim(), 10); if (p) live.add(p); }
    } catch {}
  }
  return live;
}
...
// (data/mobile-ws-port-<PID>.json) containing {"pid":<N>,"port":<N>,"sessionId":<id>}.
// Returns array of {pid, port, sessionId} for windows whose process is STILL
// ALIVE. A closed window's leftover port file is treated as dead and deleted,
// so it stops appearing as a live/controllable session in the web UI.
function listRunningCrabWindows() {
  const { dir } = resolveCrabExe();
  const windows = []; // {pid, port, sessionId}
  const pidsWithPort = new Set();
  const livePids = liveCrabPidSet();

  // 1) Read port discovery files, but only keep entries whose PID is alive.
  if (dir) {
    ...
    try {
      for (const f of fs.readdirSync(dataDir)) {
        ...
        let info = null;
        try { info = JSON.parse(fs.readFileSync(full, 'utf8')); } catch {}
        // If we know the live-PID set (non-empty) and this PID isn't in it, the
        // window is gone — remove the stale file and skip it.
        const pidAlive = livePids.size === 0 ? true : (info && livePids.has(info.pid));
        if (info && info.pid && info.port && pidAlive) {
          windows.push({ pid: info.pid, port: info.port, sessionId: info.sessionId || null });
  ...
  // 2) Live CloseCrab processes without a port file (old builds): list them so
  //    the header count stays accurate, but they have no port (no control).
  for (const pid of livePids) {
    if (!pidsWithPort.has(pid)) windows.push({ pid, port: null, sessionId: null });
  }
...
class SessionManager {
  ...
  attach(sessionId) {
    ...
    const win = wins.find((w) => w.sessionId === sessionId) || null;
  ...
  list() {
    ...
    // Match a live window to its transcript by sessionId (written into the port
    // discovery file by CloseCrab). Falling back to index-matching was the old
    // bug: it marked the wrong transcripts "live" and left closed windows
    // showing up as controllable.
    const winBySession = new Map();
    for (const w of runningWindows) {
      if (w.sessionId) winBySession.set(w.sessionId, w);
    }
    const { items } = listTranscripts();
    const existing = items.map((i) => {
      const win = winBySession.get(i.sessionId) || null;
      const alive = !!win;
      ...
      return {
        ...
        alive,
        kind: (alive && port) ? 'control' : 'attached',
