...
async function startServer(config) {
  ...
  // SAFETY: destructive "kill PC process" endpoints are blocked unless the
  // request comes from the local machine (or CLOSECRAB_ALLOW_KILL=1 is set).
  // A phone on the network must never be able to terminate the PC's CloseCrab.
  function isLocalRequest(req) {
    const ip = (req.ip || req.connection?.remoteAddress || '').replace('::ffff:', '');
    return ip === '127.0.0.1' || ip === '::1' || ip === 'localhost';
  }
  function killAllowed(req) {
    return process.env.CLOSECRAB_ALLOW_KILL === '1' || isLocalRequest(req);
  }
  ...
  // Live CloseCrab windows with their individual WebSocket ports.
  // Once CloseCrab-Unified is rebuilt with dynamic port support, each window
  // exposes its own WS on a unique port. The frontend can connect directly
  // to send prompts / receive output (full control, not just mirror).
  app.get('/api/windows', (req, res) => {
    const SM = require('./session/manager');
    // Access the internal listRunningCrabWindows (exported via require cache)
    const windows = SM.listRunningCrabWindows ? SM.listRunningCrabWindows() : [];
    res.json(windows);
  });
  ...
  app.post('/api/kill-crab', (req, res) => {
    if (!killAllowed(req)) {
      return res.status(403).json({ ok: false, message: 'Killing PC processes is disabled for remote clients.' });
    }
    let killed = [];
  ...
  // Stop ONLY the backend process that THIS web app spawned via /api/start-backend.
  // It must never touch CloseCrab windows the user started on the PC directly.
  // We kill by the tracked PID (+ child tree on Windows), not by image name.
  app.post('/api/stop-backend', (req, res) => {
    if (!backendProcess || !backendProcess.pid) {
      return res.json({
        ok: false,
        message: 'No web-started backend to stop. Windows you opened on the PC are left running.'
      });
    }
    ...
    try {
      ...
      res.json({ ok: true, message: `Web-started backend (PID ${pid}) stopped` });
    } catch {
      backendProcess = null;
      backendStartedAt = null;
