...
// Process \r: each line keeps only the last segment after \r
function processCarriageReturn(data) {
  return data.split('\n').map(line =>
    line.includes('\r') ? line.split('\r').pop() : line
  ).join('\n');
}

function appendToTerminal(data) {
  const el = document.getElementById('term-output');
  const html = ansiToHtml(processCarriageReturn(data));
  if (!html) return;
