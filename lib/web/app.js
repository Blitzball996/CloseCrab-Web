...
// === Terminal Output - simple working version ===
let userScrolling = false;
let scrollTimer = null;
let lastLineSpan = null;
...
function appendToTerminal(data) {
  ...
  // Split by \r to handle spinner overwrites
  // If data contains \r (not \r\n), the last \r segment replaces the current line
  const hasCarriageReturn = data.includes('\r') && !data.startsWith('\r\n');

  if (hasCarriageReturn) {
    // Split by \r, take the last non-empty segment
    const segments = data.split('\r');
    const lastSegment = segments[segments.length - 1];
    const html = ansiToHtml(lastSegment);

    if (lastLineSpan) {
      // Replace the last line content
      lastLineSpan.innerHTML = html;
    } else {
      lastLineSpan = document.createElement('span');
      lastLineSpan.innerHTML = html;
      el.appendChild(lastLineSpan);
    }
  } else {
    // Normal append
    ...
    // If there's a newline, reset lastLineSpan tracking
    if (data.includes('\n')) {
      lastLineSpan = null;
    }
