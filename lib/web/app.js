...
function renderSessions(sessions) {
  ...
  el.innerHTML = watermark + sessions.map(s => {
    ...
    const tag = attached
      ? '<span class="kind-tag mirror">MIRROR</span>'
    ...
    return `<div class="session-card ${attached?'attached':''}" onclick="openSession('${s.id}', ${attached})"><div class="row"><span>${t} ${tag}</span><span class="status ${s.alive?'alive':'dead'}">${statusTxt}</span></div><div class="dir">${label} ${meta}</div></div>`;
  }).join('');
}
...
function openSession(id, attached) {
  currentSessionId = id;
  currentSessionReadOnly = !!attached;
  termReady = false;
  outputBuffer = '';
  ...
  setTimeout(() => connectWS(id), attached ? 200 : 1500);
}
