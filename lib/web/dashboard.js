...
async function killAllProcesses() {
  if (!confirm('Kill ALL CloseCrab processes? This will stop everything.')) return;

  const btn = document.getElementById('dash-btn-kill');
  btn.disabled = true;
  btn.textContent = 'Killing...';

  try {
    const res = await fetch('/api/kill-all', {
      method: 'POST',
      headers: apiHeaders()
    });
    const data = await res.json();
  ...
  btn.disabled = false;
  btn.textContent = 'Kill All Processes';
}
